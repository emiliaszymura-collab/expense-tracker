...
async function getAuthorizationCode(userId, scopes = 'accounts:read,transactions:read') {
  const token = await getAppToken('authorization:grant');
  const res = await axios.post(
    `${BASE}/api/v1/oauth/authorization-grant`,
    {
      user_id: userId,
      scope: scopes,
      id_hint: userId,
      actor_client_id: 'df05e4b379934cd09963197cc855bfe9', // Tink Link actor
    },
    { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' } }
  );
